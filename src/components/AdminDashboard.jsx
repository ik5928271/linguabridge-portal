...
export default function AdminDashboard({ callLogs = [], appointments = [] }) {
  ...
  const handleDeleteReceipt = (receiptId) => {
    if (!window.confirm('Are you sure you want to permanently delete this payment receipt?')) return;
    fetch(`/api/payment-receipts/${receiptId}`, {
      method: 'DELETE'
    })
      .then(res => res.json())
      .then(data => {
                    ...
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2.5 rounded-xl bg-slate-800 hover:bg-red-500/20 text-slate-400 hover:text-red-400 border border-slate-700 transition cursor-pointer"
