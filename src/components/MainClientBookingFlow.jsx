    ...
    <div className="max-w-7xl mx-auto px-4 py-4 space-y-5">
            ...
            className={`flex flex-col items-center cursor-pointer transition ${
              currentStep === item.step 
                ...
                : item.step < currentStep 
                  ...
                  : 'text-slate-300 dark:text-slate-300 font-medium hover:text-white'
            }`}
          >
              ...
              {(() => {
                ...
                return (
                  <button
                    ...
                    className={`language-card ${isCustomLangSelected ? 'is-selected' : ''} p-3.5 rounded-2xl border text-left transition flex flex-col justify-between gap-2.5 relative cursor-pointer ${
                      isCustomLangSelected
                        ? 'bg-purple-900/35 border-purple-400 ring-2 ring-purple-400 shadow-lg shadow-purple-500/20'
                        : 'bg-slate-900/90 border-purple-500/40 hover:border-purple-400 hover:bg-slate-800/90'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 w-full">
                      <div className="overflow-hidden min-w-0 flex-1">
                        <p className={`text-sm font-extrabold truncate ${isCustomLangSelected ? 'text-purple-200' : 'text-white'}`}>
                          {isCustomLangSelected ? selectedLanguage : '+ 100+ More Languages'}
                        </p>
                        <p className="text-[11px] text-purple-300 font-semibold truncate mt-0.5">
                          {isCustomLangSelected ? '✓ Selected from 100+ Directory' : 'Browse Full A–Z Directory'}
                        </p>
                      </div>
                      <span className="text-purple-400 text-base font-bold shrink-0">
                        🌐
                      </span>
                    </div>
      ...
      {currentStep === 4 && (
        <div className="max-w-3xl mx-auto glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          ...
          {/* 🔒 AUTHENTICATION GATE: PROMPT CLIENT TO SIGN IN OR SIGN UP BEFORE PAYMENT IF NOT LOGGED IN */}
          {!currentUser ? (
            <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-br from-brand-950/80 via-slate-900 to-indigo-950/80 border-2 border-brand-500/50 shadow-2xl space-y-4">
              <div className="flex items-start gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-brand-500/20 text-brand-400 flex items-center justify-center shrink-0 ring-2 ring-brand-500/30">
                  <Lock className="w-6 h-6 text-brand-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-wider text-brand-400 bg-brand-500/10 px-2.5 py-0.5 rounded-full border border-brand-500/30">
                      Client Account Required
                    </span>
                  </div>
                  <h3 className="text-base sm:text-lg font-bold text-white mt-1">
                    Please Sign In or Sign Up Before Payment
                  </h3>
                  <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                    To attach your prepaid minutes, booking records, and 3-party session rooms directly to your secure account, please sign in or create a client account before proceeding with payment.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-slate-800/80">
                <button
                  type="button"
                  onClick={() => onOpenAuth && onOpenAuth('signin', 'host')}
                  className="py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs flex items-center justify-center gap-2 border border-slate-700 transition shadow cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={() => onOpenAuth && onOpenAuth('signup', 'host')}
                  className="py-3 px-4 rounded-xl bg-gradient-to-r from-brand-600 via-indigo-600 to-teal-600 hover:from-brand-500 hover:to-teal-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-brand-500/30 transition transform hover:-translate-y-0.5 cursor-pointer"
                >
            ...
            <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-[10px] uppercase font-bold text-emerald-400 block">Logged In Client Account:</span>
                  <span className="font-bold text-white">{currentUser.name || currentUser.email} {currentUser.org ? `(${currentUser.org})` : ''}</span>
                </div>
              </div>
      ...
      {is100LangModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md overflow-y-auto p-3 sm:p-6 flex justify-center items-start animate-fadeIn">
          <div className="bg-slate-900 border border-slate-700/80 rounded-3xl p-6 sm:p-7 max-w-3xl w-full shadow-2xl space-y-4 max-h-[90vh] flex flex-col my-auto">
                  ...
                  return (
                    <button
                      ...
                      className={`language-card ${isSel ? 'is-selected' : ''} p-3 rounded-2xl border text-left transition flex flex-col justify-between gap-1.5 cursor-pointer ${
                        isSel
                          ? 'bg-purple-600/30 border-purple-400 ring-2 ring-purple-400 shadow-md'
                          : 'bg-slate-950/80 border-slate-800 hover:bg-slate-800 hover:border-slate-700'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2 w-full">
                        <div className="overflow-hidden min-w-0 flex-1">
                          <p className={`text-xs font-bold truncate ${isSel ? 'text-purple-200' : 'text-white'}`}>
                            {lang.name}
                          </p>
